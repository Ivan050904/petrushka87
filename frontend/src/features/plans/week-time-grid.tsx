...
const DAY_COLUMN_MIN_WIDTH = 80;
const GRID_MIN_WIDTH = TIME_GUTTER + DAY_COLUMN_MIN_WIDTH * 7;
      ...
      <div ref={scrollRef} className="scrollbar-subtle min-h-0 flex-1 overflow-auto lg:min-h-[520px]">
        <div style={{ minWidth: GRID_MIN_WIDTH }}>
          <div className="sticky top-0 z-30 flex shrink-0 border-b border-border bg-muted/30">
            <div
              className="sticky left-0 z-40 shrink-0 border-r border-border/70 bg-card"
              style={{ width: TIME_GUTTER, height: DAY_HEADER_HEIGHT }}
            />
            <div
              className="grid flex-1"
              style={{ gridTemplateColumns: `repeat(7, minmax(${DAY_COLUMN_MIN_WIDTH}px, 1fr))` }}
            >
              {weekDays.map((day) => {
          ...
          <div className="flex" style={{ minHeight: TOTAL_GRID_HEIGHT }}>
            ...
            <div
              ...
              style={{ gridTemplateColumns: `repeat(7, minmax(${DAY_COLUMN_MIN_WIDTH}px, 1fr))`, minHeight: TOTAL_GRID_HEIGHT }}
            >
              {weekDays.map((day) => {
                const isToday = isSameDay(day.date, today);
                return (
